...
export default function NewWarehouseType({
  ...
  const onSubmit = async (data) => {
    if (isUpdate) {
      const baseUrl =
        ...
        "/api/warehouse/type/" +
        initialdata.id;
      makePutRequest(setLoading, baseUrl, data, "Warehouse Type", redirect);
    } else {
      const baseUrl = config.apiPrefix + config.apiHost + "/api/warehouse/type";
      makePostRequest(setLoading, baseUrl, data, "Warehouse Type", reset);
    }
  };
  ...
  return (
    <div>
      <FormHeader
        title={isUpdate ? "Update Warehouse Type" : "New Warehouse Type"}
