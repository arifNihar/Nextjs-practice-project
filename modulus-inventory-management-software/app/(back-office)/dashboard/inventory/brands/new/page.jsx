...
export default function NewBrand({ initialdata = {}, isUpdate = false }) {
  ...
  const onSubmit = async (data) => {
    if (isUpdate) {
      const baseUrl =
        config.apiPrefix + config.apiHost + "/api/brands/" + initialdata.id;
      makePutRequest(setLoading, baseUrl, data, "Brand", redirect);
    } else {
      const baseUrl = config.apiPrefix + config.apiHost + "/api/brands";
      makePostRequest(setLoading, baseUrl, data, "Brand", reset);
    }
  };
